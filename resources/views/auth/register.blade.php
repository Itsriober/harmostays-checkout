...
<head>
    ...
    <title>Register - HarmoStays Secure Payment</title>
...
<body class="gradient-bg min-h-screen flex items-center justify-center p-4">
    <div class="w-full max-w-md mx-auto glass-effect rounded-3xl shadow-xl p-8 animate__animated animate__fadeInDown">
        <div class="flex flex-col items-center mb-8">
            ...
            <h2 class="text-2xl font-bold text-white mt-2">Register for HarmoStays</h2>
        </div>
        <form method="POST" action="{{ route('register') }}" class="space-y-6">
            @csrf
            <div>
                <label class="block text-white font-semibold mb-1">Name</label>
                <input type="text" name="name" required autofocus class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-harmostays-orange">
            </div>
            <div>
                <label class="block text-white font-semibold mb-1">Email</label>
                <input type="email" name="email" required class="w-full px-4 py-2 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-harmostays-orange">
            </div>
            <div>
            ...
            <button type="submit" class="w-full bg-harmostays-pink text-white font-bold py-3 rounded-lg hover:bg-pink-700 transition hover-lift animate__animated animate__pulse animate__infinite">Register</button>
        </form>
        <div class="mt-6 text-center">
            <span class="text-white">Already have an account?</span>
            <a href="{{ route('login') }}" class="text-harmostays-green font-semibold hover:underline ml-1">Login</a>
        </div>
    </div>
</body>
</html>
